...
def she_2d_rk4(u0, T, r=1.0, dt=1e-3, record_steps=100):
    """
    2D Swift-Hohenberg equation solver using spectral method + RK4 time stepping.
    ∂u/∂t = r u - (1 + Δ)^2 u - u^3
    ...
    Parameters:
        ...
        r: control parameter
        dt: time step
        record_steps: number of saved steps
    Returns:
        sol: [batch, N, N, record_steps+1] tensor of u in physical space
        times: [record_steps+1] tensor of time values
    """
    device = u0.device
    batch, N, _ = u0.shape
    ...
    # Laplacian and operators
    lap = -(kx ** 2 + ky ** 2)
    lap2 = lap ** 2
    # Linear operator in Fourier space
    L = (r - 1) - 2 * lap - lap2
    L = L[None, :, :]  # broadcast for batch
    ...
    def nonlinear(u_phys):
        return -torch.fft.fft2(u_phys ** 3)
