...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.num_features = self.embed_dim = args.width
        self.mlp_ratio = 2
        img_size = args.size
        patch_size = 2
        self.act = ACTIVATION[args.act]
        self.patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, in_chans=self.in_channels + 3, embed_dim=self.out_channels * patch_size + 3, out_dim=self.embed_dim,act=args.act)
        self.latent_size = self.patch_embed.out_size
        self.pos_embed = nn.Parameter(torch.zeros(1, self.embed_dim, self.patch_embed.out_size[0], self.patch_embed.out_size[1]))
        ...
        self.blocks = nn.ModuleList([
            Block(mixing_type=self.mixing_type,modes=self.modes,
                width=self.embed_dim, mlp_ratio=self.mlp_ratio, channel_first = True, n_blocks=self.n_blocks,double_skip=False, h=h, w=w,act = args.act)
        ...
        self.cls_head = nn.Sequential(
            ...
            nn.Linear(self.embed_dim, self.embed_dim),
            self.act,
            nn.Linear(self.embed_dim, self.n_cls)
        )
