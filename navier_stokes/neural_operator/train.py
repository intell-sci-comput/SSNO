...
def test(net, test_data, final_t, print_step=15):
    ...
    for i in range(gth.shape[1]):
        ...
        x = pre[:, i].reshape(num, -1)
        y = gth[:, i].reshape(num, -1)
...
def train(args, net):
    ...
    data_dict_path = os.path.join(data_path, f'log/ns_val_re_{re}.txt')
    with open(data_dict_path, 'r') as file:
        data_dict = json.load(file)
    data_ratio = data_dict['record_ratio']
    data_size = data_dict['s']
    final_t_val = data_dict['T']
    sub_x = data_size // size

    data_dict_path = os.path.join(data_path, f'log/ns_test_re_{re}.txt')
    with open(data_dict_path, 'r') as file:
        data_dict = json.load(file)
    final_t_test = data_dict['T']

    print_step_val = int(print_step / final_t_test * final_t_val) 
    ...
    train_data = torch.load(os.path.join(data_path, f'dataset/ns_train_re_{re}')).float()[:num_train,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, :].to(device)
    test_data = torch.load(os.path.join(data_path, f'dataset/ns_test_re_{re}')).float()[...,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, :].to(device)
    val_data = torch.load(os.path.join(data_path, f'dataset/ns_val_re_{re}')).float()[...,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, :].to(device)
    ...
    save_dir = os.path.join(data_path, f"log_model/experiment_results_re_{args.re}_model_{model}")
    ...
    for step in range(args.num_iterations + 1):
        net.train()
        start_time = time.time()
        input_data = torch.zeros(batch_size, 1, size, size, dim, device=device)
        output_data = torch.zeros(batch_size, num_step2, size, size, dim, device=device)
        ...
        if args.use_pi==1 and sparse_coeff > 0:
        ...
        if step % 100 == 0:
            ...
            print("Validation ----------------")
            val_error, _, _, _, val_time = test(net, val_data, final_t_val, print_step=print_step_val)
            if sparse_coeff > 0:
            ...
            if val_error < best_val_error:
                ...
                print("Test ----------------")
                test(net, test_data, final_t_test, print_step=print_step)
                torch.save({
    ...
    print("Validation ----------------")
    final_val_error, val_mean_corr, val_hct_09, val_hct_08, _ = test(net, val_data, final_t_val, print_step=print_step_val)
    print("Test ----------------")
    final_test_error, test_mean_corr, test_hct_09, test_hct_08, _ = test(net, test_data, final_t_test, print_step=print_step)
    ...
    csv_path = os.path.join(data_path, "log_results",
                            f"experiment_results_re_{args.re}_size_{size}_dt_{dt}_model_{model}.csv")
    args_dict = vars(args)
