...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.__name__ = 'CNext'
        self.blocks_per_stage = 2
        self.blocks_at_neck = 2
        self.n_spatial_dims=3
        self.dim_in = args.dim
        self.dim_out = args.dim
        self.features = args.width
        self.stages = args.n_layers

        encoder_dims = [self.features * 2**i for i in range(self.stages + 1)]
        decoder_dims = [self.features * 2**i for i in range(self.stages, -1, -1)]
        encoder = []
        decoder = []
        self.in_proj = conv_modules[self.n_spatial_dims](
            self.dim_in, self.features, kernel_size=3, padding=1
        )
        self.out_proj = conv_modules[self.n_spatial_dims](
            self.features, self.dim_out, kernel_size=3, padding=1
        )
