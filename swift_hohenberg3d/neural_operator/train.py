...
def train(args, net):
    ...
    data_dict_path = os.path.join(data_path, f'log/sh3d_train.txt')
    ...
    train_data = torch.load(os.path.join(data_path, f'dataset/sh3d_train.pt'), map_location='cpu')[:num_train,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, ::sub_x, :].to(device)
    test_data = torch.load(os.path.join(data_path, f'dataset/sh3d_test.pt'), map_location='cpu')[...,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, ::sub_x, :].to(device)
    val_data = torch.load(os.path.join(data_path, f'dataset/sh3d_val.pt'), map_location='cpu')[...,
                 ::int(data_ratio // model_ratio), ::sub_x, ::sub_x, ::sub_x, :].to(device)
    ...
    for step in range(args.num_iterations + 1):
        ...
        if sparse_coeff > 0 and args.use_pi==1:
